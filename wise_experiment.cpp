...
void do_queries(string graph_file, string query_file, string threshold_file, string output_file, double factor, int k){
	...
	for (int j = 0; j < d; j++){
		...
		for (int i = 1; i <= n; i++){
			...
			do {
				getline(res,line);
			} while (line.substr(0,28) != "Candidate Generation Time : ");
			auto topk_result = topk(G, s, t, k, stats, ofs, stod(line.substr(28),nullptr) * factor);
			...
			ofs <<fixed << "Candidate Generation Time : " << stats.candidate_generation << " seconds" << endl;
			ofs << fixed << "Probability Computation Time : " << stats.probability_computation << " seconds" << endl;
			ofs << endl;
		}
		ofs << "Average Candidate Generation Time for " << h << " hops : " << t_c/n << " seconds" << endl;
		ofs << "Average Probability Computation Time for " << h << " hops : " << t_p/n << " seconds" << endl;
		ofs << "Average Total Time for " << h << " hops : " << (t_c+t_p)/n << " seconds" << endl;
